...
def graph_map(region_scans, map_config): # By accepting the list of region scans instead of a list of chunks, we can do really interesting things
  ...
  map[:] = map_config['colors']['default'] # Set the base color for the map
  ...
  for region_scan in region_scans:
    ...
    colormap_reference = map_config['colors']['regions']['non_inhabited_colormap'] # Set the default region colormap (non inhabited)
    
    if is_inhabited:
      colormap_reference = map_config['colors']['regions']['inhabited_colormap'] # In case the region is habited change the colormap
    ...
    for chunk in chunks:
      ...
      if map_config['realtime_graph']['enabled']: # If the user decided to watch the map graph in real time
        if graphing_index % map_config['realtime_graph']['update_interval'] == 0: # We skip updating the map graph until we encounter a valid graphing index
          height, width, channels = map.shape # Get the size of the map
          largest_axis = max(height, width) # Find the largest axis
          
          scale_factor = (map_config['realtime_graph']['max_size'] / largest_axis) # Determine a scale factor to resize the map
  ...
  # Resize both axis
  final_width = round(width * map_config['scale_factor'])
  final_height = round(height * map_config['scale_factor'])
  
  final_map = cv2.resize(map, (final_width, final_height)) # Scale the map to match the defined scale factor
  
  map_output = path.join(map_config['output_directory'], map_config['file_name']) # Create the complete output directory path based on the map name and the output directory
  cv2.imwrite(map_output, final_map) # Write the final map with the provided name
  
  print('Map [{0}] saved to directory [{1}]'.format(map_config['file_name'], map_config['output_directory']));